  ...
  const share = async () => {
    ...
    const text =
      sprintScore !== undefined
        ? `I scored ${sprintScore} in Globli's 60-second Sprint. Beat me here: ${duelUrl}`
        : `Globli: ${acc}% accuracy, +${xp} XP${state.streak > 0 ? `, ${state.streak}-day streak` : ""}. Think you can beat me?`;
    try {
      if (navigator.share) {
